...
function usePopover(props: EnhDatePickerProps<EnhDateTypeClear, EnhDatePrimitive[]>): any {
  return reactive({
    trigger: 'click',
    placement: 'bottom',
    hideAfter: 0,
...
export function useDatePickerEnhanced(
  props: EnhDatePickerProps<EnhDateTypeClear, EnhDatePrimitive[]>,
  emits: any,
) {
  let itemClickTimes = 0
  let lastClickIndex = -1
  let preUpdateModelValue = Array.from<[number, number]>({ length: props.modelValue?.length }).fill([0, 0])
        ...
        .map((val, index) => {
          ...
          if (valiDateAbbrStr(props.type, val).test) {
            return props.valueFormat
              ? getDateWithFormat(props.type, val, 'abbr', props.enhWantEnd, props.valueFormat)
              : getDate(props.type, val, 'abbr', props.enhWantEnd)
          }
          else {
            return props.valueFormat
              ? getDateWithFormat(props.type, localModelValue.value[index], 'array', props.enhWantEnd, props.valueFormat)
              : getDate(props.type, localModelValue.value[index], 'array', props.enhWantEnd)
          }
        })
        .sort((a, b) => +new Date(a) - +new Date(b))
  ...
  const panelItems = ref<EnhDatePickerPanelItem[][]>(Array.from<EnhDatePickerPanelItem[]>({ length: props.modelValue.length }).fill([]))
  const panelType = ref<EnhDateTypeClear[]>(Array.from<EnhDateTypeClear>({ length: props.modelValue.length }).fill(props.type))
  ...
  const panelItemClick = (index: number, item: EnhDatePickerPanelItem) => {
    ...
    if (item.type === 'year' && props.type !== 'year') {
      ...
      if (!props.enhAllowSame && itemClickTimes === 2 && (Number(preUpdateModelValue[lastClickIndex].join('')) === Number([item.year, item[props.type] || 0].join('')))) {
        // 有效点击次数 - 1
        itemClickTimes -= 1
        return
      }
  ...
  watch(panelValue, (newV) => {
    ...
    const newModelValue = preUpdateModelValue
      .map((item, _index) => {
        return props.valueFormat
          ? getDateWithFormat(props.type, item, 'array', props.enhWantEnd, props.valueFormat)
          : getDate(props.type, item, 'array', props.enhWantEnd)
      })
      .sort((a, b) => +new Date(a) - +new Date(b))
  ...
  watch(() => popover.visible, (newVal: boolean) => {
    if (newVal) {
      panelValue.value = getPanelValue(props.type, localModelValue.value)
      panelItems.value = Array.from<EnhDatePickerPanelItem[]>({ length: props.modelValue.length }).fill([])
      panelType.value = Array.from<EnhDateTypeClear>({ length: props.modelValue.length }).fill(props.type)
      generatePanelItems()
    }
    else {
      // 状态重置
      itemClickTimes = 0
      lastClickIndex = -1
      preUpdateModelValue = Array.from<[number, number]>({ length: props.modelValue.length }).fill([0, 0])
    }
  })
