  ...
  const CommonPicker = vue.defineComponent({
    ...
    setup(_, { emit, slots, expose }) {
      ...
      return () => vue.h('div', { 'data-testid': 'mock-common' }, [
        ...
        vue.h('button', { 'data-testid': 'emit-update', 'onClick': () => emit('update:model-value', 'v') }),
...
describe('datePickerEnhanced expose and events', () => {
  it('calls exposed methods and forwards picker events', async () => {
    ...
    const Host = defineComponent({
      ...
      template: '<DatePickerEnhanced ref="pickerRef" type="quarteryear" :model-value="new Date()" :teleported="false" @panel-change="panelChange" @focus="focus" @visible-change="visibleChange" @update:model-value="updateModelValue" @change="change" @blur="blur" @clear="clear" @calendar-change="calendarChange" />',
    })
    ...
    expect(updateModelValue).toHaveBeenCalledWith('v')
